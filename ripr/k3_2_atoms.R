...
optimise_two_atoms <- function(n, q, grid = 50L) {
  ...
  best_w1 <- function(log_comps) {
    stats::optimize(
      function(w1) kl_loss(log_comps, w1),
      c(1e-6, 1 - 1e-6)
    )$minimum
  }
  ...
  objective <- function(u) {
    ...
    log_comps <- cbind(
      log_multinom_all(null_boundary_3(s1)),
  ...
  log_Pw <- log(rowSums(exp(
    sweep(log_comps, 2L, log(c(w1, 1 - w1)), "+") - row_max
  ))) +
    row_max
