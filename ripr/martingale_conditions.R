...
ns <- as.integer(regmatches(
  files,
  regexpr("(?<=optim_n_)\\d+", files, perl = TRUE)
))
ord <- order(ns)
files <- files[ord]
ns <- ns[ord]
...
eval_e <- function(x, n, log_wts_row) {
  ...
  log_Pw_f <- torch_logsumexp(
    mnom_logpmf(x_full, log_ws, n) + log_wts_row,
    dim = 2L
  )
...
conditional_check <- lapply(seq_len(length(ns) - 1L), function(i) {
  ...
  log_wts_n <- torch_tensor(
    weights[[i]][which.min(results[[i]]$final_max_exp), , drop = FALSE],
    device = device,
    dtype = dtype
  )$log() # (1, C)
  log_wts_n1 <- torch_tensor(
    weights[[i + 1L]][
      which.min(results[[i + 1L]]$final_max_exp),
      ,
      drop = FALSE
    ],
    device = device,
  ...
  e_next_mat <- torch_stack(
    lapply(1:3, function(cat) {
...
results_df <- data.frame(
  ...
  max_excess_theta_idx = sapply(
    conditional_check,
    `[[`,
    "max_excess_theta_idx"
  ),
  max_excess_en = sapply(conditional_check, `[[`, "max_excess_en"),
  prop_max_excess = sapply(conditional_check, `[[`, "prop_max_excess"),
  prop_max_excess_xn_idx = sapply(
    conditional_check,
    `[[`,
    "prop_max_excess_xn_idx"
  ),
  prop_max_excess_theta_idx = sapply(
    conditional_check,
    `[[`,
    "prop_max_excess_theta_idx"
  ),
